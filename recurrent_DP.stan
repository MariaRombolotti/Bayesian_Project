data{
  ...
  real<lower=0> b_lambda; // scale parame \\ter lambda_k
  real<lower=0> sigma_b;// varianza beta
  real a_sigma;
  real b_sigma;
  real mu_mu;//mean of the gaussian G_0
  real sigma_mu;//var of the gaussian G_0
  }
...
model{
 ...
 for(i in 1:M){
    for(c in 1:C){
      ps[c]=log(pi[c])+normal_lpdf(w[i]|mu_cl[c],sigma_cl[c]);
    }
 ...
 //target += gamma_lpdf(w_new | 1/eta, 1/eta);
  ...
  target += inv_gamma_lpdf(sigma_cl | a_sigma, b_sigma);
  target += normal_lpdf(mu_cl | mu_mu, sigma_mu);
  target += gamma_lpdf(alpha | 6, 1);
  target += beta_lpdf(v | 1, alpha);  
}
